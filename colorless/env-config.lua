...
-- Build hotkeys on config parameters
------------------------------------------------------------------------------
function env:init(args)
    ...
    if beautiful.naugthy then
        naughty.config.presets.normal   = bwm.util.table.merge(beautiful.naughty.base, beautiful.naughty.normal)
        naughty.config.presets.critical = bwm.util.table.merge(beautiful.naughty.base, beautiful.naughty.critical)
        naughty.config.presets.low      = bwm.util.table.merge(beautiful.neughty.base, beautiful.naughty.low)
...
env.tagtip = function(t)
    ...
    if bwm.util.table.check(beautiful, "widget.layoutbox.name_alias") then
...
env.wrapper = function(widget, name, buttons)
    local margin =  bwm.util.table.check(beautiful, "widget.wrapper") and beautiful.widget.wrapper[name]
